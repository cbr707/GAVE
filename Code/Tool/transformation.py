...
def random_affine(npimages):
    ...
    zoom = 0.7  # you can modify it

    dh, dw = int(h*1/zoom - h), int(w*1/zoom - w)

    shear = np.deg2rad(random.uniform(-25, 25))

    center_shift = np.array((w, h)) / 2.0
...
# def random_affine(npimages, zoom=0.7):
#     h, w = npimages[0].shape[:2]
#     dorotate = np.deg2rad(random.uniform(-180, 180))
#     shear = np.deg2rad(random.uniform(-25, 25))
#
...
#         + transform.AffineTransform(rotation=dorotate, scale=(zoom, zoom), shear=shear)
#         + transform.SimilarityTransform(translation=center)
#     )
#
#     output_shape = (h, w)  # 保持原尺寸
#     transformed = []
#     for i, img in enumerate(npimages):
#         order = 0 if i in (1, 2) else 3
#         warped = transform.warp(
#             img,
#             inverse_map=tform.inverse,
#             output_shape=output_shape + img.shape[2:],
#             order=order,
#             preserve_range=True,
#             mode='constant',
#             cval=0
